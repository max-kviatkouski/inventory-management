          ...
          <table class="orders-table">
            ...
            <tbody>
              <tr v-for="order in submittedOrders" :key="order.id">
                ...
                <td class="col-lead-time">{{ leadTimeDays(order) }} {{ t('restocking.days') }}</td>
                <td class="col-status">
                  <span :class="['badge', getOrderStatusClass(order.status)]">{{ t('status.submitted') }}</span>
                </td>
...
export default {
  ...
  setup() {
    ...
    const leadTimeDays = (order) =>
      Math.round((new Date(order.expected_delivery) - new Date(order.order_date)) / 86400000)
